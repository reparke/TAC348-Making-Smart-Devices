...
/*
    ...
    2) create pin label
      --> GLOBAL CONSTANTS which refer ANY pins we plan to use in our project /
   code
      --> global means defined OUTSIDE of a function at the TOP of our code
            accessible EVERYWHERE in our code
        (in contrast, LOCAL variables are defined INSIDE a function and ONLY
...
void loop() {
    // int brightness = 0;  //wouldn't work because it would reset to 0 EVERY
    // LOOP cycle v1 - digital write digitalWrite(PIN_LED, HIGH); //send 3.3v to
    // pin A2 (anode of the LED ) to make it turn on delay(1000); //1 sec or
    // 1000 ms digitalWrite(PIN_LED, LOW);   //send 0v to pin A2 to make it turn
    // off delay(2000);  //2 sec or 2000 ms

    // v2 - analog write
    //  0 - 255 control how long the voltage to the LED is high vs. low
    //     "duty cycle"  PWM 0 --> 0%  and PWM 255 --> 100% always on
    //     --> effect of this is that 0-255 will make the LED brighter or less
    //     bright

    //   analogWrite(PIN_LED, 0)    --> same as digitalWrite(PIN_LED, LOW) -->
    //   LED OFF analogWrite(PIN_LED, 255)    --> same as digitalWrite(PIN_LED,
    //   HIGH)   --> LED ON (full brightness)
    ...
    /*
      v3 auto increase
        --> have led off, slowly increase in brightnesss until max, then go back
      to off, repeat
    ...
    */
    ...
    // how can we make the pot control the LED brightness?
    //  if statements, connect pot to LED directly
    //  the microcontroller control everything
    // read the pot val, and use that value to contorl the led
    //  pot value range (PWM): 0-255
    //  adc digital range:     0-4095

    /*
      C++
      / is division
        int / int  ---> int (throws away dec)

        float / int --> float

    */

    // int pwmVal = potVal / 4095 * 255;  // math is incorrect

    // int pwmVal = potVal / 4095.0 * 255;  // this works
...
/*
  ...
  --
  PWM -- ANALOG OUTPUT
    simulating analog voltage
    analogWrite(PIN, PWM_Value)
      range of PWM value: 0-255
        0  -> 0V (LED is off)
        255 -> 3.3v (LED is on)
  ...
  ADC - analog digital conversion
    ...
    converting 0-3.3V input to 0-4095 digital value
      0 - 4095

      4095 is 2^12 (-1)
      photon has a 12 bit ADC
*/
