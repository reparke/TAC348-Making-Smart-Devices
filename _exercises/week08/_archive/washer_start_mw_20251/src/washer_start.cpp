...
void getCyclePosition() {
    // convert potentiometer value to cycle?
    // int potVal = analogRead(POT_PIN);
    // if (potVal >= 0 && potVal < 1365) {  // 1365 = 4095/3
    ...
    // alternate way
    int potVal = analogRead(POT_PIN);
    int mappedVal = map(potVal, 0, 4095, 0, 2);  // mappedVal goes from 0 to 2
...
void updateNextState() {
    /*
    in general, what kinds of things happen in this function?
        - check currentTime
        - check any inputs (cycle and switch)
        - use logic to determine the next state and update state var
        - update any outputs

        stage 1: implement economy cycle

        Rob's code will first SWITCH based on STATE
    */
    // all the system inputs
    unsigned long currMillis = millis();
    getCyclePosition();  // updates the cycle
