...
/*
    when the photon / accel were sitting on our desks,
        why were getting 0.04g in x and y dir, and 0.86g in z?
        -we know that grav is 9.8m/s2 pulling down
        -->
        -not perfectly level
        -not calibrated
*/
...
void loop() {
    //     adxl.readAndCalcAccel();  // reads current x,y,z accel and calculates
    //     G's

    //     /* After reading, six class variables are updated: x, y, z, cx, cy,
    //     and cz. Those are the raw, 12-bit values (x, y, and z) and the
    //     calculated acceleration's in units of g (cx, cy, and cz). */
    //     displayExampleGraphs();
    //     delay(500);

    // calc values
    //  adxl.readAndCalcAccel()  <--- call this fn EVERYTIME before read the
    //  sensor
    //  adxl.readAndCalcAccel();
    /*
        we have access to the x y z measurement
        adxl.x   .y     .z     <---- are raw measurement- don't use
        adxl.cx   .cy   .cz    <--- calc in terms of g's 9.8 m/s2

        how could we use the accel values to control the RGB LED color?
    */
    // int r = map(adxl.cx, -0.86, 0.86, 0.0, 255.0);
    // int g = map(adxl.cy, -0.86, 0.86, 0.0, 255.0);
    // int b = map(adxl.cz, -0.86, 0.86, 0.0, 255.0);

    // int r = map(abs(adxl.cx), 0.0, 0.86, 50.0, 255.0);
    // int g = map(abs(adxl.cy), 0.0, 0.86, 50.0, 255.0);
    // int b = map(abs(adxl.cz), 0.0, 0.86, 50.0, 255.0);
    ...
    /*
        the accel is GREAT at measuring ONE thing (tap, double tap, activty)
        the accel is pretty at measuring TWO things
        the accel is AWFUL at measuring all three at the same time
            very hard to set all the parameters correctly to measure each
       reliably
    */

    // if (adxl.readActivity() == true) {
    //     Serial.println("activity count = " + String(activityCount++));
    // }
    // else
    if (adxl.readTap() == true) {
        Serial.println("tap count = " + String(tapCount++));
    } else if (adxl.readDoubleTap() == true) {
        Serial.println("double tap count = " + String(doubleTapCount++));
    }
}
