...
/*
    ...
    --> lets test these work before we dive to building

    how can we quickly test if the pots and buttons are wired properly?
    -->
*/
const int PIN_POT1 = A0;
const int PIN_POT2 = A1;
const int PIN_BUTTON = D2;
// no pin labels needed for OLED
...
/*
    show hello world on the oled (DONT USE printTitle())

    Etch a sketch
        two pots, oled, button
        pots are knobs to draw dot
        button is erase (don't worry about latch)
        oled screen

        button erase --> oled.clear(PAGE)

        two pots
            input range: 0-4095
            output range: screen is 64 x 48 pixels
            get max height from .getLCDHeight
            get max width from .getLCDWidth
...
// from looking online at forums and docs
...
void loop() {
    ...
    //convert potVal --> pixel
    //convert 0-4095  --> 0-64   or 0-48

    //map converts one linear range of number to another
    //syntax
...
//     Serial.println("pot 1 = " + String(pot1Val) + ", pot2 = " +
