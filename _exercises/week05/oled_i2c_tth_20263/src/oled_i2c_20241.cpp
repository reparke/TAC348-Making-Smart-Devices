...
/*
    display hello world on the screen without using printTitle
*/
...
/*
etch a sketch
    two pots, 1 button, 1 oled

    pots are going to knobs (one for x, one for y)
    button is to erase the screen
    oled will display
    ==
    button - how will this erase the current drawing?
        oled.clear(PAGE)

    pots - control x and y direction -- how?
        scale / map /convert the input values to the screen dim

    oled - how do we display the dot / drawing?
        oled.pixel(x,y)
...
void loop() {
    ...
    // oled.clear(PAGE); //clear  the screen
    // oled.setFontType(0);
    // oled.setCursor(0,0); //start at the top left
    ...
    // map / scale the pot values to the screen
    // what is xPot? --> ADC value 0-4095
    // screen dimensions are 0 - getLCDHeight or width
...
/*
    hardware is 2 pots, 1 button, 1 oled

    --> first thing: let just make sure the HW works
    lets create a test function
*/

// void testInputs() {
//     int pot1Val = analogRead(PIN_POT1);
//     int pot2Val = analogRead(PIN_POT2);
//     int buttonVal  = digitalRead(PIN_BUTTON);

//     Serial.println("pot1 = " + String(pot1Val) + ", pot2 = " +
//     String(pot2Val) + ", button = " + String(buttonVal));
// }
