...
void setup() {
    ...
    //clear the sparkfun logo
    oled.clear(PAGE);
}


/*
    phase 1: comment out the code in loop()
        show "hello world" on the oled WITHOUT using printTitle()

    phase 2: etch a sketch
        two pots, button, oled screen
        
        pots are knobs (one x dir, one for y dir)
        button is to erase the screen
        oled is screen

        button - how can it erase the screen?
            oled.clear(PAGE)

        pots - will control x and y coords for drawing
            how?
            map the analogValues from pots to the screen dimension

        oled - how do we draw a dot
            oled.pixel(xCoord, yCoord)
...
void loop() {
    ...
    // oled.setFontType(0);  //0, 1 2 or 3 -- optional
    // oled.setCursor(0,0);
    // oled.print("Hello World");
    // oled.display();             // must always do this for anything to appear

    int xPot = analogRead(PIN_POT1);
    int yPot = analogRead(PIN_POT2);
    int buttonVal = digitalRead(PIN_BUTTON);

    //map the ADC value to pixel
    //map funct syntax
    //outVal = map(inVal, inMin, inMax, outMin, outMax)
    int xCoord = map(xPot, 0, 4095, 0, oled.getLCDWidth() - 1 );
    int yCoord = map(yPot, 0, 4095, 0, oled.getLCDHeight() - 1);
