...
/*
    display hello world on the screen WITHOUT using print Title
*/
...
/*
    build a etch a sketch
        two pots, button, screen

    pots are knobs (one for x dir, one for y dir)
    button is to reset
    oled is screen
======
    button - how does this erase the screen?
        oled.clear(PAGE)

    pots - control x and y dir for drawing--how?
         map the pot values to our coordinates

    oled - how do draw a dot /pixel?
        oled.pixel(x, y)


        green check - go bcak to Breakouts
        red x - stay quietly for a few
*/
...
void setup() {
    ...
    oled.clear(PAGE); //remove the sparkfun logo
}
...
void loop() {
    ...
    // oled.display(); //must call display to make text show up

    int xPot = analogRead(PIN_POT1);
    int yPot = analogRead(PIN_POT2);
    // what now?
    //  map ADC val [0-4095] to pixel val [0-oled.getLCDHeight (or Width)]
    //  go ahead and do the math to convert if you want OR we could MAP function
    ...
    // what should to have button erase screen?
    int buttonVal = digitalRead(PIN_BUTTON);  // skip latch for now
    if (buttonVal == LOW) {
        oled.clear(PAGE);
    }
}
...
// /*
//     hardware: oled, 2 pots, 1 button
//     --> before we start, let's quickly test the hardware
// */
