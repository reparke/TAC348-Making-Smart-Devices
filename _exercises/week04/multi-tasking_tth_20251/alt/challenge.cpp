...
const unsigned long INTERVAL_LED1_OFF = 700;  // 300 ms
...
void loop() {
    ...
    if (led1State == false) {
        if (currMillis - prevMillisLed1 > INTERVAL_LED1_OFF) {
        ...
        if (currMillis - prevMillisLed1 > INTERVAL_LED1_ON) {
            // this means the timer has elapsed!
            // in this IF, always update our timer
            prevMillisLed1 = currMillis;
            led1State = false;
    ...
    if (prevButtonVal == HIGH &&
        ...
        led2BlinkActive = !led2BlinkActive;
    }
